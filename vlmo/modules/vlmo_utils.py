...
from vlmo.gadgets.my_metrics import Accuracy, VQAScore, Scalar, AdvVQAAccuracy, AdvVQAScore
    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                setattr(pl_module, f"{split}_vqa_score", AdvVQAScore())
                setattr(pl_module, f"{split}_{k}_accuracy", AdvVQAAccuracy())
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "nlvr2":
                if split == "train":
                ...
                # setattr(pl_module, f"{split}_{k}_cos_similarity", Scalar())
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "itm":
