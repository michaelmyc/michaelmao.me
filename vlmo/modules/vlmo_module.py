...
class VLMo(pl.LightningModule):
    def __init__(self, config):
        ...
        if config["loss_names"]["itm"] > 0 or config["loss_names"]["itm_infer"] > 0:
        ...
        ## Adversarial VQA Classification
        if self.hparams.config["loss_names"]["adv_vqa"] > 0:
            # vs = 5
            # self.adv_vqa_classifier = nn.Sequential(
            #     nn.Linear(self.num_features, self.num_features * 2),
            #     nn.LayerNorm(self.num_features * 2),
            #     nn.GELU(),
            #     nn.Linear(self.num_features * 2, vs),
            # )
            # self.adv_vqa_classifier.apply(objectives.init_weights)
            vs = self.hparams.config["vqav2_label_size"]
            self.vqa_classifier = nn.Sequential(
                nn.Linear(self.num_features, self.num_features * 2),
                nn.LayerNorm(self.num_features * 2),
        ...
        ## Adversarial VQA Classification
        if self.hparams.config["loss_names"]["itm_embed"] > 0:
            # vs = 5
            # self.adv_vqa_classifier = nn.Sequential(
            #     nn.Linear(self.num_features, self.num_features * 2),
            #     nn.LayerNorm(self.num_features * 2),
            ...
            es = self.hparams.config["embedding_size"]
            self.itm_embedding = nn.Sequential(
                nn.Linear(self.num_features, self.num_features * 2),
                nn.LayerNorm(self.num_features * 2),
                nn.GELU(),
                nn.Linear(self.num_features * 2, es),
            )
            self.itm_embedding.apply(objectives.init_weights)
    ...
    def forward(self, batch):
        ...
        # Image Text Matching without hard negative, infer only
        if "itm_infer" in self.current_tasks:
            ret.update(objectives.compute_itm(self, batch))

        # Image Text Matching without hard negative, infer only
        if "adv_vqa" in self.current_tasks:
            ret.update(objectives.compute_adv_vqa(self, batch))

        # Image Text Matching without hard negative, infer only
        if "itm_embed" in self.current_tasks:
            ret.update(objectives.compute_itm_embed(self, batch))

        # # Image Text Matching without hard negative, infer only
        # if "itm_embed_infer" in self.current_tasks:
        #     ret.update(objectives.compute_adv_vqa(self, batch))
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.hparams.config["output_dir"]:
