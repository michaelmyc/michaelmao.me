    ...
    def list_to_cpu(lst):
        ...
        for v in lst:
            if isinstance(v, torch.Tensor):
                new_lst.append(v.cpu())
            elif isinstance(v, list):
                new_lst.append(list_to_cpu(v))
            else:
                new_lst.append(v)
        return new_lst
...
def compute_itm(pl_module, batch):
    ...
    itm_logits = pl_module.itm_score(infer_pos["cls_feats"])
    itm_probs = torch.sigmoid(itm_logits)

    ret = {
        "itm_logits": itm_logits.cpu(),
        "itm_probs": itm_probs.cpu(),
        "iid": batch['iid'],
        "qid": batch['qid']
    }
...
def compute_itm_embed(pl_module, batch):
    # batch = {k: v for k, v in batch.items()}

    infer_imag = pl_module.infer_image(batch, mask_image=False)
    infer_text = pl_module.infer_text(batch, mask_text=False)

    imag_embedding = pl_module.itm_embedding(infer_imag["cls_feats"])
    text_embedding = pl_module.itm_embedding(infer_text["cls_feats"])

    device = pl_module.device
    
    cos_loss = nn.CosineEmbeddingLoss()
    itm_embedding_loss = cos_loss(imag_embedding, text_embedding, torch.ones(imag_embedding.shape[0]).to(device))

    # imag_embedding_vec = imag_embedding.view(imag_embedding.shape[0], 1, imag_embedding.shape[1])
    # text_embedding_vec = text_embedding.view(text_embedding.shape[0], 1, text_embedding.shape[1])
    # itm_embedding_loss = torch.mean(torch.abs(torch.bmm(imag_embedding_vec, text_embedding_vec)))

    ret = {
        "imag_embedding": imag_embedding.cpu(),
        "text_embedding": text_embedding.cpu(),
        "itm_embedding_loss": itm_embedding_loss,
    }
...
# def compute_adv_vqa(pl_module, batch):
#     infer = pl_module.infer(batch, mask_text=False, mask_image=False)
#     adv_vqa_logits = pl_module.adv_vqa_classifier(infer["cls_feats"])
#     adv_vqa_targets = torch.tensor(batch["answer_type_labels"], dtype=torch.float).to(pl_module.device)

#     adv_vqa_loss = (
#         F.binary_cross_entropy_with_logits(adv_vqa_logits[..., 0], adv_vqa_targets[..., 0]) + 
#         F.binary_cross_entropy_with_logits(adv_vqa_logits[..., 1:], adv_vqa_targets[..., 1:]) * 3000 # balance out data imbalance
#     )

#     ret = {
#         "adv_vqa_loss": adv_vqa_loss,
#         "adv_vqa_logits": adv_vqa_logits,
#         "adv_vqa_targets": adv_vqa_targets,
#     }

#     phase = "train" if pl_module.training else "val"
#     loss = getattr(pl_module, f"{phase}_adv_vqa_loss")(ret["adv_vqa_loss"])
#     accuracy = getattr(pl_module, f"{phase}_adv_vqa_accuracy")(
#         ret["adv_vqa_logits"], ret["adv_vqa_targets"]
#     )
#     pl_module.log(f"adv_vqa/{phase}/loss", loss)
#     pl_module.log(f"adv_vqa/{phase}/accuracy", accuracy)
...
def compute_adv_vqa(pl_module, batch):
    infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    adv_vqa_logits = pl_module.vqa_classifier(infer["cls_feats"])
    adv_vqa_targets = torch.tensor(batch["answer_type_labels"], dtype=torch.float).to(pl_module.device)
    is_adversarial = adv_vqa_targets[..., 0] == 0
    vqa_targets = torch.zeros(
        len(adv_vqa_logits), pl_module.hparams.config["vqav2_label_size"]
    ).to(pl_module.device)

    vqa_labels = batch["vqa_labels"]
    vqa_scores = batch["vqa_scores"]

    for i, (_label, _score, _adversarial) in enumerate(zip(vqa_labels, vqa_scores, is_adversarial)):
        if _adversarial:
            _score = torch.zeros(len(_label))
        for l, s in zip(_label, _score):
            vqa_targets[i, l] = s

    adv_vqa_loss = 0
    if is_adversarial.sum() > 0:
        adv_vqa_loss += F.binary_cross_entropy_with_logits(adv_vqa_logits[is_adversarial], vqa_targets[is_adversarial]) * vqa_targets.shape[1] * is_adversarial.sum() * 100
    if is_adversarial.sum() != is_adversarial.numel():
        adv_vqa_loss += F.binary_cross_entropy_with_logits(adv_vqa_logits[torch.bitwise_not(is_adversarial)], vqa_targets[torch.bitwise_not(is_adversarial)]) * vqa_targets.shape[1] * (is_adversarial.numel() - is_adversarial.sum())
    ...
    loss = getattr(pl_module, f"{phase}_adv_vqa_loss")(ret["adv_vqa_loss"])
    accuracy = getattr(pl_module, f"{phase}_adv_vqa_accuracy")(
        ret["adv_vqa_logits"], ret["vqa_targets"]
    )
    score = getattr(pl_module, f"{phase}_vqa_score")(
        ret["adv_vqa_logits"], ret["vqa_targets"], ret["is_adversarial"]
    )
    pl_module.log(f"adv_vqa/{phase}/loss", loss)
    pl_module.log(f"adv_vqa/{phase}/accuracy", accuracy)
    pl_module.log(f"adv_vqa/{phase}/score", score)
