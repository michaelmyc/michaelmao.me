...
def make_arrow(root, dataset_root):
    ...
    for split, questions in zip(
        ["train", "val", "test", "test-dev", "adversarial-train", "adversarial-val"],
        [
            questions_train2014,
            questions_val2014,
            questions_test2015,
    ...
    for split, annots in zip(
        ...
        for q in tqdm(annots):
            ...
            _annot[q["image_id"]][q["question_id"]].append(
                {"labels": labels, "scores": scores, "answer_type": [1, 0, 0, 0, 0]}
            )
    ...
    for split, annots in zip(
        ...
        for q in tqdm(annots):
            ...
            _annot[q["image_id"]][q["question_id"]].append(
                {"labels": labels, "scores": scores, "answer_type": q['answer_type'], "is_adversarial": True}
            )
    ...
    for split in [
        ...
        with pa.OSFile(f"{dataset_root}/vqav2_{split}_adv.arrow", "wb") as sink:
            with pa.RecordBatchFileWriter(sink, table.schema) as writer:
                writer.write_table(table)

    table = pa.ipc.RecordBatchFileReader(
        pa.memory_map(f"{dataset_root}/vqav2_val_adv.arrow", "r")
    ).read_all()
    ...
    with pa.OSFile(f"{dataset_root}/vqav2_trainable_val_adv.arrow", "wb") as sink:
        with pa.RecordBatchFileWriter(sink, df1.schema) as writer:
            writer.write_table(df1)

    with pa.OSFile(f"{dataset_root}/vqav2_rest_val_adv.arrow", "wb") as sink:
