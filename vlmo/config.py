...
def _loss_names(d):
    ret = {
        ...
        "itm_infer": 0, # image-text matching loss
        "adv_vqa": 0, # adversarial vqa classification loss
        "itm_embed": 0, # image text matching siamese loss (with embedding)
    }
    ret.update(d)
    return ret
...
@ex.named_config
def task_itm_infer_base_vg():
    exp_name = "itm_infer_base_vg"
    datasets = ["vqa"]
    loss_names = _loss_names({"itm_infer": 1})
    batch_size = 1024
    whole_word_masking = True
    learning_rate = 2e-4
    train_transform_keys = ["square_transform_randaug"]
    ...
    test_only = True
    output_dir = "/workspace/research/vlmo-retrieval-base/output"
...
# ----------------------- Adversarial VQA configs -----------------------

@ex.named_config
def task_finetune_adv_vqa_base():
    exp_name = "finetune_adv_vqa_base_image480"
    datasets = ["adv_vqa"]
    train_transform_keys = ["square_transform_randaug"]
    loss_names = _loss_names({"adv_vqa": 1})
    batch_size = 128
    max_epoch = 10
    max_steps = None
    warmup_steps = 0.1
...
@ex.named_config
def task_finetune_adv_vqa_only_base():
    exp_name = "finetune_adv_vqa_only_base_image480"
    datasets = ["adv_vqa_only"]
    train_transform_keys = ["square_transform_randaug"]
    loss_names = _loss_names({"adv_vqa": 1})
    batch_size = 128
    max_epoch = 10
...
@ex.named_config
def task_test_vqa_base_image480():
    exp_name = "test_vqa_base_image480"
    datasets = ["vqa"]
    loss_names = _loss_names({"vqa": 1})
    val_transform_keys = ["square_transform"]
    image_size = 480
...
@ex.named_config
def task_finetune_adv_vqa_no_answer_base():
    exp_name = "finetune_adv_vqa_base_image480_no_answer"
    datasets = ["adv_vqa"]
    train_transform_keys = ["square_transform_randaug"]
    loss_names = _loss_names({"adv_vqa": 1})
    batch_size = 128
...
# ==== Siamese Image Text Matching ====

@ex.named_config
def task_finetune_itm_with_embedding():
    exp_name = "finetune_itm_with_embedding_image480"
    datasets = ["vg"]
    train_transform_keys = ["square_transform_randaug"]
    loss_names = _loss_names({"itm_embed": 1})
    batch_size = 128
    max_epoch = 10
    max_steps = None
    warmup_steps = 0.1
...
@ex.named_config
def task_generate_itm_embedding():
    exp_name = "generate_itm_embedding_image480"
    datasets = ["vg", "vqa", "adv_vqa"]
    train_transform_keys = ["square_transform_randaug"]
    loss_names = _loss_names({"itm_embed": 1})
    batch_size = 128
    max_epoch = 10
