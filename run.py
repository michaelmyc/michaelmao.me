...
def main(_config):
    ...
    if _config["output_dir"]:
        output = [{k: v.cpu().numpy().tolist() if isinstance(v, torch.Tensor) else v for k, v in batch.items()} for batch in model.results]
        current_time = datetime.now().astimezone(pytz.timezone("America/Los_Angeles")).strftime("%Y%m%dT%H%M")
        fname = Path(_config["output_dir"])/f'{exp_name}_seed{_config["seed"]}_from_{_config["load_path"].split("/")[-1][:-5]}_{current_time}_{id(model)}.json'
